...
function handleUri(uri)
	...
	local optionValue = config.GetGlobalOption("browser.cmd")
...
--		handleUri(util.String(bp.Cursor:GetSelection()))
	else
...
function init()
	config.RegisterGlobalOption("browser", "cmd", defaultCmdOption)
	
	config.MakeCommand("browse", browse, config.NoComplete)
	config.AddRuntimeFile("browserutils", config.RTHelp, "help/browserutils.md")
	
	-- config.TryBindKey("F9", "lua:browserutils.browser", false)
end